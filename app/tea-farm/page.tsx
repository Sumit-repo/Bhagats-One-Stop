...
export default function TeaFarmPage() {
  ...
  const { expenses, earnings, loading, addEntry, deleteExpense, deleteEarning, updateExpense, updateEarning } = useTeaFarm();
  ...
  const handleExportCsv = () => {
    ...
    downloadCsv(`tea-farm-${selectedYear === 'all' ? 'all-years' : selectedYear}.csv`, rows);
  };
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-4 sm:p-8">
            ...
            <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white tracking-tight mb-1">Tea Farm Management</h1>
              ...
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                ...
                {availableYears.length > 0 && (
                  <select
                    ...
                    className="px-4 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 text-sm font-bold text-gray-700 dark:text-slate-300 rounded-2xl focus:ring-2 focus:ring-emerald-500/40 outline-none cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-[2rem] font-black text-base hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-200 dark:shadow-none hover:scale-[1.02] active:scale-95"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <SummaryCard icon={Wallet} title="Total Earnings" value={`₹${totalEarnings.toFixed(2)}`} variant="green" />
              <SummaryCard
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="text-center p-6 bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-100 dark:border-emerald-500/10 rounded-2xl">
                  <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 mb-2 uppercase tracking-wider">Total Earnings</p>
                  <p className="text-3xl font-black text-emerald-600 dark:text-emerald-400">₹{totalEarnings.toFixed(2)}</p>
