...
import { X, Receipt, Leaf, Info, Calendar as CalendarIcon, Package, DollarSign, Pencil } from 'lucide-react';
import { TeaFarmExpense, TeaFarmEarning, ExpenseCategory } from '@/models/TeaFarm';

export type TeaEditTarget =
  | { kind: 'expense'; entry: TeaFarmExpense }
  | { kind: 'earning'; entry: TeaFarmEarning };

interface EditEntryModalProps {
  target: TeaEditTarget | null;
  onClose: () => void;
  onSaveExpense: (id: string, data: Partial<Omit<TeaFarmExpense, 'id' | 'created_at'>>) => Promise<void>;
  onSaveEarning: (id: string, data: Partial<Omit<TeaFarmEarning, 'id' | 'created_at'>>) => Promise<void>;
}
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[2rem] p-6 sm:p-8 max-w-md w-full shadow-2xl transition-colors relative overflow-hidden max-h-[90dvh] overflow-y-auto">
        <div className={`absolute -top-40 -right-40 w-80 h-80 rounded-full blur-3xl pointer-events-none opacity-20 transition-colors ${isExpense ? 'bg-red-500' : 'bg-emerald-500'}`} />

        <div className="flex items-center justify-between mb-8 relative z-10">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isExpense ? 'bg-red-50 dark:bg-red-500/10' : 'bg-emerald-50 dark:bg-emerald-500/10'}`}>
              <Pencil className={`w-4 h-4 ${isExpense ? 'text-red-500' : 'text-emerald-500'}`} />
            </div>
            <div>
              ...
              <p className="text-xs font-bold text-gray-400 dark:text-slate-500 mt-1 uppercase tracking-widest">
                {isExpense ? <><Receipt className="inline w-3 h-3 mr-1" />Expense</> : <><Leaf className="inline w-3 h-3 mr-1" />Earning</>}
              </p>
            </div>
          </div>
          <button
