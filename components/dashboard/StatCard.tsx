...
export function StatCard({ icon: Icon, title, value, change, trend, variant = 'blue' }: StatCardProps) {
  ...
  return (
    <div className={`rounded-2xl p-6 ${isGreenCard ? variantStyles[variant] : 'bg-white border border-gray-100'}`}>
      <div className="flex items-start justify-between mb-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isGreenCard ? iconBgStyles[variant] : iconBgStyles[variant]}`}>
      ...
      <div>
        <p className={`text-sm font-medium mb-1 ${isGreenCard ? 'text-white/80' : 'text-gray-600'}`}>{title}</p>
        <p className={`text-3xl font-bold mb-2 ${isGreenCard ? 'text-white' : 'text-gray-900'}`}>{value}</p>
        <div className="flex items-center gap-1">
          ...
          <span className={`text-sm ${isGreenCard ? 'text-white/70' : 'text-gray-500'}`}>this week</span>
        </div>
      </div>
    </div>
  );
