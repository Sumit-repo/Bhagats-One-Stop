...
function ActionMenu({ customer, onOpenEdit, onDelete }: {
  customer: Customer;
  onOpenEdit: () => void;
  onDelete: (id: string) => Promise<void>;
}) {
      ...
      {open && (
        <div className="absolute right-0 top-10 z-50 w-52 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
          {!confirming ? (
            <>
              <button
                onClick={() => { onOpenEdit(); setOpen(false); }}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
              >
                <Pencil className="w-4 h-4 text-blue-500" /> Edit Customer
              </button>
              <button
                onClick={() => setConfirming(true)}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors border-t border-gray-100 dark:border-slate-700"
              >
...
export function CustomerTable({ customers, onFilter, onDelete, onEdit }: CustomerTableProps) {
  ...
  const handleSearch = (val: string) => { setSearch(val); onFilter({ search: val }); setPage(1); };
    ...
    <>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                {paged.length > 0 ? paged.map((customer) => (
                  <tr key={customer.id} className="group hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-5 text-right">
                      <ActionMenu
                        customer={customer}
                        onOpenEdit={() => setEditingCustomer(customer)}
                        onDelete={onDelete}
                      />
                    </td>
                  </tr>
      ...
      {editingCustomer && (
        <EditCustomerModal
          ...
          onSave={async (updated) => { await onEdit(updated); setEditingCustomer(null); }}
        />
      )}
    </>
  );
